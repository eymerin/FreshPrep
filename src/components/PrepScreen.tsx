        ...
        {pendingPreps.length === 0 && prepSessionsLogged === 0 && !success && (
          <div className="mb-6 px-4 py-5 bg-brand-surface rounded-xl border border-brand-muted/10 text-center">
            ...
            <p className="text-xs text-brand-muted/35 leading-relaxed">
              Go to Plan, pick your meals for the week, check off your shopping list, then tap "Finish Shopping."
            </p>
          </div>
        )}
        ...
        {pendingPreps.length > 0 ? (
          <div className="mt-2">
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3 bg-brand-surface border border-brand-muted/15 rounded-xl text-sm text-brand-muted/60 hover:text-brand-muted transition-colors"
            >
              <span>Log a different meal</span>
              <svg
                width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                strokeWidth="2" strokeLinecap="round"
                className={`transition-transform ${logDrawerOpen ? 'rotate-180' : ''}`}
              >
                <polyline points="6 9 12 15 18 9" />
              </svg>
            </button>
            {logDrawerOpen && <div className="mt-2">{manualForm}</div>}
          </div>
        ) : (
          manualForm
        )}
